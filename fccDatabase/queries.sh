...
echo "$($PSQL "SELECT DISTINCT(name) FROM (SELECT name, year, round FROM games LEFT JOIN teams ON games.winner_id = teams.team_id UNION SELECT name, year, round FROM games LEFT JOIN teams ON games.opponent_id = teams.team_id) fteams WHERE year='2014' AND round='Eighth-Final' ORDER By name")"
